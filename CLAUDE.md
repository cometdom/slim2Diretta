...
**DoP-aware silence (v1.4.3+)**: a DAC locked in DoP needs *valid DoP silence* during gaps, not `0x00`. Plain `0x00` breaks DoP framing (marker byte ≠ `0x05`/`0xFA`) and is not DSD idle (`0x69`), so the DAC drops DoP lock and emits a full-scale crack. The `isDoP` flag propagates to `DirettaSync` via `configureRingPCM(..., isDoP)` and the quick-resume fast path (both set `m_dopSilence`; `configureRingDSD` and the reset path clear it). `getNewStream()` emits silence through `fillSilence()`, which — when `m_cachedDopSilence` is set — tiles a DoP idle pattern (24-bit LE packed: payload `0x69`, marker alternating `0x05`/`0xFA` per frame) instead of `memset(0x00)`. The pattern is rebuilt in the `getNewStream` cold path (on format-generation change) from the cached channel count — worker-thread-local, no shared buffer, no extra RT-path locking. This covers every silence site: quick-resume flush, shutdown silence, 5 s idle release, underrun/rebuffering. Plain PCM (`0x00`) and native DSD (`0x69`) silence are unchanged. Note: only the silence emitted by `getNewStream` reaches the DAC; the ring buffer's internal `0x00` fill never does (`pop()` only returns real data), so it was left as-is.
